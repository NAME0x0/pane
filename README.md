...
- native block I/O policy, stateful Pane block-port protocol, WHP block-port classification, and host callback submission for the future guest-visible storage device,
- host-side native block I/O adapter that services allowed kernel-layout read commands from verified base/user artifacts and preserves user-disk write semantics for the upcoming data-plane milestone,
