...
Storage-backed kernel layouts require the generated Pane initramfs driver bundle and a SHA-verified `pane-block.ko` so root discovery is not treated as ready without a guest-visible block device. The generated discovery initramfs now boots a C `/init` directly, records the Pane storage/display contracts, loads `pane-block.ko` with `device_blocks=<base>,<user>` from `pane.block_devices`, waits for the declared root device, attempts to mount it at `/newroot`, and executes the real init once a Pane block device exists. The bundle also emits `pane-block.c` and `build-pane-block-module.sh`, which define the early Linux block-device contract that maps Pane's verified base OS to `/dev/pane0` and the writable user disk to `/dev/pane1`; the generated block path supports partition minors, partial reads, read-modify-write partial writes, and 32-bit port transfers so partition scanners and filesystems are not limited to byte-at-a-time host transfer size. The kernel-layout runner now requires deterministic serial milestones through `PANE_BLOCK_MODULE_LOAD_OK`, `PANE_DISPLAY_CONTRACT_DISCOVERED`, `PANE_ROOT_MOUNT_OK`, and `PANE_INIT_EXEC` before treating storage-backed Arch boot as proven progress. Use `--trace-checkpoint <path>` on long native boot probes to receive incremental JSON diagnostics, including WHP time-slice boundary evidence and the first native timer-interrupt request before Pane attempts a safe resume path.
