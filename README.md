...
- native block I/O policy, stateful Pane block-port protocol, WHP block-port classification, host callback submission, serviced-exit resume, guest-visible read response streaming, and guest-to-host write-payload collection for the future storage device,
