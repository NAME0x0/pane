...
2. runs `pane.exe --help`, `pane.exe environments`, `pane app-status`, `pane runtime --prepare --create-user-disk`, `pane runtime --register-boot-loader`, `pane runtime --register-kernel`, `pane native-kernel-plan --prepare-runtime --materialize`, `pane native-preflight`, `pane native-boot-spike`, `pane native-boot-spike --run-kernel-layout`, `pane launch --runtime pane-owned --dry-run`, `pane doctor --no-write`, `pane init --dry-run`, and `pane onboard --dry-run`,
