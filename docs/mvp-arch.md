...
10. Run `pane runtime --register-base-image C:\path\to\arch-base.img --expected-sha256 <64-char-sha256> --require-native-root-disk` when you have a Pane-approved Arch raw disk image with a detectable Linux root partition to copy into the app-owned runtime store. Images registered without an expected digest are recorded but treated as untrusted, and images registered without `--require-native-root-disk` are not accepted for the native storage-backed Arch boot path.
