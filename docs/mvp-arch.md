...
6. Run `pane runtime --register-native-boot-set --register-base-image C:\path\to\arch-base.img --expected-sha256 <64-char-sha256> --register-kernel C:\path\to\vmlinuz-linux --kernel-expected-sha256 <64-char-sha256> --register-initramfs C:\path\to\initramfs-linux.img --initramfs-expected-sha256 <64-char-sha256> --kernel-cmdline "console=ttyS0 panic=-1"` when you have a trusted Arch raw disk, Linux kernel, initramfs, and serial-safe cmdline to prevalidate and copy into Pane's runtime store together.
