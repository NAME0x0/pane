...
8. Run `pane native-kernel-plan --materialize` to write the deterministic boot layout for boot params, cmdline, kernel, optional initramfs, mapped framebuffer/input queue ranges, and the initial E820 guest memory map before the WHP kernel-entry milestone starts.
