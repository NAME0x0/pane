...
4. Run `pane native-preflight --prepare-runtime --json` when you want to create the native runtime boundary and inspect Windows Hypervisor Platform readiness plus the runtime artifact blockers for the future boot-to-serial spike.
...
6. Run `pane runtime --write-native-boot-set-manifest-template C:\path\to\pane-native-boot-set.json` when you need the reviewed JSON shape that a reproducible Arch artifact builder should fill in.
7. Run `.\write-native-boot-set-manifest.ps1 -BaseImage C:\path\to\arch-base.img -Kernel C:\path\to\vmlinuz-linux -Initramfs C:\path\to\initramfs-linux.img -OutputPath C:\path\to\pane-native-boot-set.json -Force` from a packaged Pane directory when you want Pane's helper to compute SHA-256 values and write the handoff manifest for those artifacts.
8. Run `pane runtime --register-native-boot-set-manifest C:\path\to\pane-native-boot-set.json` when a reproducible Arch artifact builder has emitted a boot-set manifest. The manifest path is preferred because it carries the raw disk, Linux kernel, initramfs, SHA-256 values, and serial-safe cmdline together. If you are registering artifacts manually, run `pane runtime --register-native-boot-set --register-base-image C:\path\to\arch-base.img --expected-sha256 <64-char-sha256> --register-kernel C:\path\to\vmlinuz-linux --kernel-expected-sha256 <64-char-sha256> --register-initramfs C:\path\to\initramfs-linux.img --initramfs-expected-sha256 <64-char-sha256> --kernel-cmdline "console=ttyS0 panic=-1"` instead.
9. Run `pane runtime --write-initramfs-driver` to generate the Pane guest-side native storage discovery bundle for the future initramfs/root handoff.
10. Run `pane native-kernel-plan --prepare-runtime --materialize` to prepare the runtime boundary if needed and write the deterministic boot layout for boot params, cmdline, kernel, optional initramfs, mapped framebuffer/input queue ranges, and the initial E820 guest memory map before the WHP kernel-entry milestone starts.
11. Run `pane native-boot-spike --json` to inspect the first executable WHP milestone in safe plan mode. Add `--execute` when you intentionally want Pane to create and tear down the temporary WHP partition/vCPU; add `--execute --run-fixture` when you also want Pane to map guest memory, set registers, run controlled guest code, and observe the COM1 serial I/O exit. Add `--execute --run-boot-loader` when you want to run the verified runtime-provided loader candidate. Add `--execute --run-kernel-layout` only with a controlled small candidate to prove the materialized kernel layout is consumed by the WHP runner.
12. Run `pane runtime --register-base-image C:\path\to\arch-base.img --expected-sha256 <64-char-sha256> --require-native-root-disk` only when you intentionally want to register the base disk separately. Images registered without an expected digest are recorded but treated as untrusted, and images registered without `--require-native-root-disk` are not accepted for the native storage-backed Arch boot path.
13. Run `pane launch --runtime pane-owned --dry-run` when you want to exercise the future native-runtime path without invoking WSL, `mstsc.exe`, or XRDP. This currently reports the remaining host, base-image, boot-engine, and display-engine blockers.
14. Run `pane doctor --de xfce --no-write --no-connect` when you want an initial diagnostic pass that does not create Pane workspace state or PaneShared.
15. Run `"strong-password" | pane onboard --username archuser --password-stdin` as the preferred first-run CLI flow. Pane will initialize or reuse the managed Arch distro, create or repair the Arch login, enable the required WSL settings, and verify launch readiness.
16. Run `pane terminal` when you want shell-level work such as dotfiles or later password changes. Use `pane terminal --user root` for package installation, repair work, or any admin task unless you have configured sudo yourself inside Arch.
17. Run `pane launch --de xfce`.
18. If you want to verify reconnect readiness later, run `pane doctor --de xfce --skip-bootstrap`. That path now checks both the Pane-managed XRDP session assets and whether the Arch user's XFCE config/cache directories exist, are owned correctly, and are writable.
19. Use `pane connect` to reopen the saved `.rdp` profile. Pane will use direct localhost when it is available and fall back to pane-relay when Windows cannot reach the WSL XRDP port directly.
20. Use `pane share` to open or print the Windows-side directory exposed in Arch as `~/PaneShared`.
21. Keep the default durable PaneShared mode when the folder is user data. Use `pane launch --shared-storage scratch` when the shared folder should be disposable with the session workspace.
22. Use `pane logs` when bootstrap or XRDP readiness fails.
23. Run `pane bundle` before asking for support so the current diagnostics are packaged once.
24. Use `pane init` plus `pane setup-user` only when you want to split onboarding into separate lifecycle steps or adopt a specific existing Arch distro.
