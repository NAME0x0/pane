...
- delayed WHP capability loading backed by Microsoft-generated `windows-sys` ABI definitions, so unsupported hosts still receive preflight diagnostics instead of a process-load failure,
- the Control Center,
...
3. Runtime-backed serial test image: materialize `serial-boot.paneimg` under the Pane runtime, map it as guest memory, configure vCPU registers, run it, decode the complete `PANE_BOOT_OK` COM1 banner across repeated I/O exits, stop at that deterministic completion boundary (or an earlier reported HLT), unmap memory, and tear everything down cleanly.
...
- `pane native-boot-spike --prepare-runtime --execute --run-fixture` can load the runtime-backed serial boot image, map guest memory, set registers, run guest code, decode the complete deterministic `PANE_BOOT_OK` serial banner, stop at that bounded completion boundary without waiting on a halted vCPU, unmap memory, and release all WHP resources,
...
- materialized kernel layouts attach the verified base OS image and Pane user disk when both exist, including both the current Pane block-port diagnostic bridge and the replacement virtio-blk backend contract that maps the read-only Arch base disk to `vda`, the writable Pane user disk to `vdb`, and the preferred root handoff to `pane.virtio_root=/dev/vdaN` when a root partition is known,
- storage-backed layouts reserve a Pane-owned modern virtio-MMIO block register page at `0x0dfc0000`; Pane now has tested Linux-compatible modern virtio-blk feature advertisement, feature/queue register readback, width-aware config-field reads, unsupported feature masking with FEATURES_OK refusal until negotiation is valid, DRIVER_OK gating until FEATURES_OK is accepted, absent reporting for non-existent selected queues, QueueReady runtime-state reset, queue-notify execution gating until DRIVER_OK and QueueReady are both set, batched split-virtqueue descriptor-chain draining for read/write/status completion, malformed descriptor-chain consumption with used-ring error completion, empty-kick tolerance, guest-visible block-error completion without aborting WHP emulation, accurate used-ring transfer lengths, read-only base-disk write denial, live WHP instruction-emulator callback routing with callback detail reporting into that MMIO service boundary, and WHP virtio IRQ requests after queue completion,
- `pane native-preflight` and `pane native-boot-spike` publish a typed crosvm-style device-loop route contract for WHP exits, making the current serial, block diagnostic, virtio-MMIO block, legacy platform I/O, timer, display, input, and CPU-control ownership explicit before those handlers are extracted into real devices,
...
- `pane native-boot-spike --prepare-runtime --execute --run-kernel-layout` now uses the stricter Arch boot-attempt readiness contract before loading the kernel layout into WHP: the verified root disk, user disk, kernel plan, initramfs driver bundle, pane-block module, discovery initramfs, materialized layout, framebuffer/input contracts, runtime config, native manifest, and WHP host checks must all be ready. When they are ready, Pane maps boot params, cmdline, kernel, optional initramfs, non-overlapping RAM regions, reserved initramfs E820 placement, a boot-protocol GDT, reserved APIC MMIO stub pages, framebuffer memory, input queue memory, and the Pane block DMA window, then selects the protected-mode Linux entry probe with boot params in `rsi`; storage-backed Linux layouts must report module-load, display-contract, root-mount, and init-exec serial milestones before being treated as Arch userspace boot progress, and `--trace-checkpoint <path>` writes an initial fail-fast checkpoint before WHP execution and incremental JSON diagnostics while the guest is running. The WHP runner now guards long vCPU runs, treats canceled runs as explicit time-slice boundaries, applies a longer storage-backed live-run budget while preserving the no-progress escape hatch, tracks PIC/PIT state enough to derive the timer vector, requests the first native timer interrupt through `WHvRequestInterrupt`, performs one guarded post-interrupt resume, captures interrupt-delivery state through `WHvGetVirtualProcessorRegisters`, records block-I/O resume checkpoints, and uses shared-memory payload transfer only after positive guest negotiation. The generated `pane-block.ko` also caps successful per-transfer status logging after the first diagnostic samples, and the generated discovery init uses Pane's root filesystem hint and mounts the immutable base root read-only first so root-mount probing matches the actual storage ownership model. The June 19 live trace proved the generated module loads, reaches root mount, completes host-backed reads, and reports an ext4 signature failure because Pane incorrectly cleared port-stream response bytes into an unnegotiated DMA window; the host path now preserves those bytes unless the guest reports `PANE_BLOCK_SHARED_BUFFER_OK`,
- the June 20 rust-vmm queue validation reached `PANE_BLOCK_MODULE_LOAD_OK` and `PANE_DISPLAY_CONTRACT_DISCOVERED`, but `/dev/vda1` timed out before appearing, no virtio interrupt was requested or acknowledged, and the diagnostic pane-block fallback serviced one base-image read before the root-mount budget expired. `PANE_ROOT_MOUNT_OK` and `PANE_INIT_EXEC` were not reached, so pane-block remains required as a labeled fallback,
