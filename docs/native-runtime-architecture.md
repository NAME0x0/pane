...
- storage-backed layouts reserve a Pane-owned modern virtio-MMIO block register page at `0x0dfc0000`; Pane now has tested Linux-compatible modern virtio-blk feature advertisement, batched split-virtqueue descriptor-chain draining for read/write/status completion, live WHP instruction-emulator callback routing into that MMIO service boundary, and WHP virtio IRQ requests after queue completion,
