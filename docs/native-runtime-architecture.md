...
- verified Pane user disk snapshots, restore validation, and portable export/import packages under the runtime snapshot/storage boundary,
...
10. Storage materialization: turn the descriptor into a durable block-device format with resize, snapshot, repair, export, and import semantics. Pane now has verified user-disk snapshot, restore, export, and import artifacts; resize and deeper repair remain explicit future increments.
