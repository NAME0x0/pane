...
- verified Pane user disk snapshots and restore validation under the runtime snapshot store for recovery/export/import milestones,
...
10. Storage materialization: turn the descriptor into a durable block-device format with resize, snapshot, repair, export, and import semantics. Pane now has verified user-disk snapshot and restore artifacts; resize, export, and import remain explicit future increments.
