...
8. CPU exit handling: classify unhandled memory-access exits as blockers with exact access type/GPA/GVA diagnostics, pass WHP default CPUID results back to the guest, maintain a minimal guest MSR state for RDMSR/WRMSR, and keep expanding the Linux boot CPU contract only when the next real exit demands it.
...
- `pane native-boot-spike --execute --run-kernel-layout` can consume that layout, map boot params, cmdline, kernel, optional initramfs, and non-overlapping RAM regions, then select the correct guest-entry contract: real-mode serial/HALT validation for the controlled candidate or a protected-mode Linux entry probe with boot params in `rsi` for a bzImage payload,
