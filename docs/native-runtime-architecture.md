...
- a materialized storage attachment in the kernel layout when the verified Arch base image and Pane user disk are present, including a guest-mapped storage contract page, sparse disk geometry, and header verification data,
...
9. Runtime artifact boot: connect the kernel path to Pane's verified Arch base image and Pane sparse user disk. The layout now carries `/dev/pane0` as the read-only base OS device and `/dev/pane1` as writable user/package/customization storage when both artifacts are verified, including a guest-mapped storage contract page, sparse logical size, block size, backing mode, and header hash.
