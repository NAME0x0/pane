...
- a first Pane-owned framebuffer contract and initialized input queue mapped into guest memory for the future app-rendered display boundary, plus host-side framebuffer/input snapshot reporting after guarded WHP runs,
...
- runtime preparation writes explicit framebuffer and input contracts, and the mapped input queue begins with a deterministic `PANEINQ1` header, so display/input work has a stable guest/device boundary instead of an undefined "draw pixels somehow" milestone,
