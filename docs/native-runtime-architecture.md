...
8. CPU exit handling: classify unhandled memory-access and MSR exits as blockers, pass WHP default CPUID results back to the guest, and keep expanding the Linux boot CPU contract only when the next real exit demands it.
