...
- a native block I/O command policy, stateful Pane block-port register protocol, WHP Pane block-port classification boundary, host callback submission path, serviced-exit resume, and guest-visible byte-stream response window for the future storage device,
- a host-side native block I/O adapter that services allowed kernel-layout read commands from verified base-image/user-disk artifacts and preserves user-disk write semantics for the upcoming write-payload milestone,
...
- a materialized storage attachment in the kernel layout when the verified Arch base image and Pane user disk are present, including a guest-mapped storage contract page, root handoff decision, sparse disk geometry, and header verification data,
