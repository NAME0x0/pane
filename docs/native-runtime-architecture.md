...
- a native block I/O command policy, stateful Pane block-port register protocol, WHP Pane block-port classification boundary, host callback submission path, serviced-exit resume, guest-visible read response streaming, and guest-to-host write-payload collection for the future storage device,
