...
- a native block I/O command policy, stateful Pane block-port register protocol, WHP Pane block-port classification boundary, and host callback submission path for the future guest-visible storage device,
- a host-side native block I/O adapter that services allowed kernel-layout read commands from verified base-image/user-disk artifacts and preserves user-disk write semantics for the upcoming data-plane milestone,
