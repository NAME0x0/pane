...
- storage-backed layouts reserve a Pane-owned modern virtio-MMIO block register page at `0x0dfc0000`; Pane now has tested Linux-compatible modern virtio-blk feature advertisement, feature/queue register readback, batched split-virtqueue descriptor-chain draining for read/write/status completion, empty-kick tolerance, guest-visible block-error completion without aborting WHP emulation, accurate used-ring transfer lengths, read-only base-disk write denial, live WHP instruction-emulator callback routing into that MMIO service boundary, and WHP virtio IRQ requests after queue completion,
