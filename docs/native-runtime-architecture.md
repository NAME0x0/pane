...
- storage-backed layouts reserve and map a Pane-owned modern virtio-MMIO block register page at `0x0dfc0000`; Pane now has tested split-virtqueue descriptor-chain execution for read/write/status completion plus a typed MMIO service boundary, with live WHP instruction-emulator callback wiring still pending,
