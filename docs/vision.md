...
- a guarded WHP guest execution milestone through `pane native-boot-spike --prepare-runtime --execute --run-fixture`,
- a verified runtime-provided boot-loader candidate path through `pane runtime --register-boot-loader` and `pane native-boot-spike --prepare-runtime --execute --run-boot-loader`,
...
- an initial Linux E820 memory-map contract with framebuffer/input queue ranges before Pane attempts real Arch boot,
- a materialized kernel boot-layout contract through `pane native-kernel-plan --prepare-runtime --materialize`,
...
Pane also does not yet boot a Pane-owned OS image from its own disk. The `pane runtime` command prepares the dedicated app space for that future engine: downloads, base OS image, sparse user disk, verified user-disk snapshots, verified snapshot restore, portable user-disk export/import packages, grow-only user-disk resize, user-disk metadata repair, runtime state, runtime config, a native-runtime manifest, a runtime-backed serial boot image, a verified boot-to-serial loader candidate slot, and a verified kernel/initramfs boot-plan slot. It can register a local Arch base image with SHA-256 metadata, require a native Arch raw disk with a detectable Linux root partition through `--require-native-root-disk`, prevalidate and register a native Arch boot set with the root disk, Linux bzImage, initramfs, and serial-safe cmdline through `--register-native-boot-set`, write a reviewed reproducible-builder manifest template through `--write-native-boot-set-manifest-template`, consume a reproducible builder handoff through `--register-native-boot-set-manifest`, register a controlled loader candidate with SHA-256 and expected serial-output metadata, register a kernel plus optional initramfs with a required `console=ttyS0` cmdline, create the Pane-owned sparse user-disk artifact, snapshot that artifact, restore it, export it, import it, grow it, and rebuild metadata from a valid disk header after validating metadata, byte count, SHA-256, and header geometry before replacing active storage. `pane native-kernel-plan --prepare-runtime --materialize` turns that verified kernel plan into the deterministic guest-memory layout for boot params copied from the bzImage setup header, cmdline, kernel, optional initramfs, a boot-protocol GDT, reserved APIC stubs, mapped framebuffer/input queue ranges, and an explicit E820 guest memory map. `pane native-preflight --prepare-runtime` creates the app-owned runtime boundary and probes the Windows Hypervisor Platform host boundary plus runtime artifact readiness for the first boot-to-serial spike. `pane native-boot-spike --prepare-runtime --execute --run-fixture` creates a temporary WHP partition/vCPU, loads `serial-boot.paneimg` from the Pane runtime, maps guest memory, configures registers, observes the complete `PANE_BOOT_OK` COM1 banner across repeated I/O exits, stops at that deterministic boundary rather than waiting on a halted vCPU, then tears everything down. `pane native-boot-spike --prepare-runtime --execute --run-boot-loader` applies the same guarded WHP execution path to a verified runtime-provided loader candidate. `pane native-boot-spike --prepare-runtime --execute --run-kernel-layout` consumes the materialized kernel layout, maps boot params, cmdline, kernel, optional initramfs, non-overlapping usable RAM regions, framebuffer memory, input queue memory, and the boot-protocol GDT, and now carries the guest-entry contract through the WHP runner: bounded real-mode serial validation for the controlled candidate or a Linux 32-bit protected-mode entry probe with boot params in `rsi` for a bzImage payload. It still does not boot Arch or render a desktop until real early Linux serial output is deterministic. `pane launch --runtime pane-owned --dry-run` exercises that future path without invoking WSL, `mstsc.exe`, or XRDP, but it intentionally stops at the current boot/display blockers instead of pretending the native engine exists.
