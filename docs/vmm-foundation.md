...
Current implementation status: the WHP binding migration is linked and advancing incrementally. Pane directly depends on Microsoft `windows-sys 0.61.2`, uses its generated Kernel32 loader bindings while preserving delayed `WinHvPlatform.dll`/`WinHvEmulation.dll` capability checks, derives core WHP capability/property/GPA-map/exit constants from generated definitions, and now uses the generated interrupt-control and GVA-translation result structures directly. Tests compare every remaining local WHP compatibility layout against the official ABI size. The partition/vCPU run loop remains behavior-compatible and dynamically resolved; register unions and emulator callback records are the next Step 1 conversion because they require coordinated field-level call-site changes.
