...
Current implementation status: the first WHP binding migration slice is linked. Pane directly depends on Microsoft `windows-sys 0.61.2`, uses its generated Kernel32 loader bindings while preserving delayed `WinHvPlatform.dll`/`WinHvEmulation.dll` capability checks, derives core WHP capability/property/GPA-map/exit constants from generated definitions, and tests every remaining local WHP compatibility layout against the official ABI size. The partition/vCPU run loop remains behavior-compatible and dynamically resolved; replacing the remaining local compatibility structs and function aliases is the next Step 1 increment.
